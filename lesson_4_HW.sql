create database if not EXISTS Lesson4_HW;
...
select shops.*, cats_join.*
from shops
JOIN (SELECT shops_id, name FROM cats
WHERE name="Murzik") AS cats_join
ON shops.id = cats_join.shops_id;

	-- 3.Вывести магазины, в которых НЕ продаются коты “Мурзик” и “Zuza”
select * from shops
join cats
on shops.id = cats.shops_id where cats.name != "Murzik" and cats.name != "Zuza";
...
-- 4. Dывести название и цену для всех анализов, которые продавались 5 февраля 2020 и всю следующую неделю.
